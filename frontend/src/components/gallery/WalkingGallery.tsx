...
export const WalkingGallery: FunctionComponent<Props> = ({ robotUrl, content }) => {
  ...
  return (
    <div className="relative w-full aspect-[4/3] bg-yellow-100 overflow-hidden flex items-center justify-center">
      ...
      />
      <p className={`absolute top-[20%] left-[10%] text-lg font-semibold`}>강남구에서 걷기좋은 거리</p>
      <img src={"/gn50/public/half_circle.png"} alt="half-circle" className="absolute top-[20%] left-[50%] -translate-x-1/2 transition-transform duration-500" style={{ transform: `rotate(${rotation - 200}deg)` }} />
      <img src={"/gn50/public/half_circle.png"} alt="half-circle" className="absolute top-[20%] right-[50%] -translate-x-1/2 transition-transform duration-500" style={{ transform: `rotate(${rotation - 90}deg)` }} />
      {/* 아이템 그리드 */}
      {isShown && <div className="flex space-x-8">
